...
public:
  ...
  void Finalize();
  void gemv_cublas(CellState *d_input1, CellTemp *d_temp1,
                   float *weights1,
                   const int gate_num, int wave_size);
...
__global__ void solve(CellState *d_output, CellState *d_state_c,
                      CellTemp *d_temp_w, CellTemp *d_temp_u,
                      CellModel *d_model);
...
void Wave::gemv_cublas(CellState *d_input1, CellTemp *d_temp1,
                       float *weights1,
                       const int gate_num, int wave_size)
{
  float alpha = 1.0f;
  ...
  int strideA = sizeof(CellModel) / sizeof(float);
  int stridex = (kLstmTimestep * sizeof(CellState)) / sizeof(float);
  int stridey = sizeof(CellTemp) / sizeof(float);
...
{
  CellState *d_input_w =
      &((CellParams *)d_cell_params)->cell_state_h[cell_idx - 1][step_idx];
  CellState *d_input_u =
      &((CellParams *)d_cell_params)->cell_state_h[cell_idx][step_idx - 1];
  CellTemp *d_gemvw_temp =
      &((CellParams *)d_cell_params)->gemvw_temp[cell_idx - 1];
  CellTemp *d_gemvu_temp =
      &((CellParams *)d_cell_params)->gemvu_temp[cell_idx - 1];
  CellModel *d_model =
      &((ModelParams *)d_model_params)->cell_model[cell_idx - 1];
  CellState *d_output =
      &((CellParams *)d_cell_params)->cell_state_h[cell_idx][step_idx];
  CellState *d_state_c =
      &((CellParams *)d_cell_params)->cell_state_c[cell_idx - 1];

  for (int gate_num = 0; gate_num < kLstmGateNumber; ++gate_num)
  {
    float *weights_w_ptr = &d_model->weights_w[gate_num][0][0];
    float *weights_u_ptr = &d_model->weights_u[gate_num][0][0];

    gemv_cublas(d_input_w, d_gemvw_temp, weights_w_ptr, gate_num, wave_size);
    gemv_cublas(d_input_u, d_gemvu_temp, weights_u_ptr, gate_num, wave_size);
  }
...
void WavefrontLSTM::Solve()
{
  ...
  */
  ...
  for (int wave_idx = 1; wave_idx <= max_wave_number; ++wave_idx)
  {
    ...
    wave_.Compute(d_model_params_, d_cell_params_, step_start_num, layer_start_num, wave_size);
  }
}
