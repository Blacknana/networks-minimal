...
public:
  ...
  void Solve(int wave_size, WaveKernelParams kernel_params);
  void Finalize();
...
__global__ void wave_compute(ModelParams *d_model_params,
                             ...
                             int layer_start_num);
__global__ void wave_solve(ModelParams *d_model_params,
                           CellParams *d_cell_params, int step_start_num,
                           int layer_start_num);
...
Wave::Wave()
{
  ...
  CUDA_CHECK(
      cudaGetFuncBySymbol(&cu_wave_solve_, (const void *)wave_solve));
}
...
void Wave::Solve(int wave_size, WaveKernelParams kernel_params)
{
  CU_CHECK(LaunchKernel(cu_wave_solve_, kGemvBlockNumber * wave_size,
                        kHiddenSize, 0, kernel_params));
}
...
void WavefrontLSTM::Solve()
{
  ...
  for (int wave_idx = 1; wave_idx <= max_wave_number; ++wave_idx)
  {
    ...
    wave_.Solve(wave_size, kernel_params);
  }
}
...
__global__ void __launch_bounds__(256, 4)
    wave_solve(ModelParams *d_model_params, CellParams *d_cell_params,
               int step_start_num, int layer_start_num)
{
  const int cell_idx = layer_start_num + blockIdx.x / kGemvBlockNumber;
  const int step_idx = step_start_num - blockIdx.x / kGemvBlockNumber;
