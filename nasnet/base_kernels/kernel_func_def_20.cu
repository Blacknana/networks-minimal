...
// Output:
//	- name: Add_520_0	type: float	shape: Shape{1, 32, 32, 32}
__device__ __forceinline__ void Add_float_float_float_cuda_Add_520_block_kernel(
...
BlockFusionKernel_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_cuda_fused_kernel_Add_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_Add_4(
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 63) {
    ...
    Add_float_float_float_cuda_Add_520_block_kernel(
        input4, input5, output1, threadIdx.x, blockIdx.x - 64, NULL);
  } else if ((int)blockIdx.x >= 128 && (int)blockIdx.x <= 191) {
    Add_float_float_float_cuda_Add_520_block_kernel(
        input15, input16, output7, threadIdx.x, blockIdx.x - 128, NULL);
  } else if ((int)blockIdx.x >= 192 && (int)blockIdx.x <= 447) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_521_block_kernel(
        input6, input7, output2, threadIdx.x, blockIdx.x - 192, NULL);
  } else if ((int)blockIdx.x >= 448 && (int)blockIdx.x <= 703) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_522_block_kernel(
        input6, input8, output3, threadIdx.x, blockIdx.x - 448, NULL);
  } else if ((int)blockIdx.x >= 704 && (int)blockIdx.x <= 959) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_522_block_kernel(
        input9, input10, output4, threadIdx.x, blockIdx.x - 704, NULL);
  } else if ((int)blockIdx.x >= 960 && (int)blockIdx.x <= 1215) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_521_block_kernel(
        input11, input12, output5, threadIdx.x, blockIdx.x - 960, NULL);
  } else if ((int)blockIdx.x >= 1216 && (int)blockIdx.x <= 1471) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_521_block_kernel(
        input13, input14, output6, threadIdx.x, blockIdx.x - 1216, NULL);
  }
}
