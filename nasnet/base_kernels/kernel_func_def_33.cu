...
void Constant_float_cuda_Constant_2593(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2593_0.bin",
...
void Constant_float_cuda_Constant_136(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_136_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_19(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_19_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2569(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2569_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_432(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_432_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_429(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_429_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
// Input:
// Output:
//	- name: Constant_254_0	type: float	shape: Shape{3, 3, 64, 1}
void Constant_float_cuda_Constant_254(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_254_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2341(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2341_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_296(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_296_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2176(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2176_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2530(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2530_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
// 32}
// Fused functions:
// Convolution_float_float_float_cuda_Convolution_627<<<dim3(2, 16, 2), dim3(16,
// 2, 8), 0, 0>>>(DepthwiseConv2dNative_621_0, Constant_2110_0,
// Convolution_627_0);
// Convolution_float_float_float_cuda_Convolution_629<<<dim3(2, 16, 2), dim3(16,
// 2, 8), 0, 0>>>(DepthwiseConv2dNative_622_0, Constant_2113_0,
// Convolution_629_0); Deduped function map: <src_function_name :
// deduped_function_name> Convolution_float_float_float_cuda_Convolution_629 :
// Convolution_float_float_float_cuda_Convolution_627
...
Convolution_float_float_float_cuda_Convolution_627_block_kernel(
...
BlockFusionKernel_float_float_float_float_float_float_cuda_Convolution_Convolution_16(
...
// 8}
// Fused functions:
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1363<<<dim3(64,
// 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1360_0, Constant_279_0,
// DepthwiseConv2dNative_1363_0);
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1364<<<dim3(64,
// 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1360_0, Constant_188_0,
// DepthwiseConv2dNative_1364_0);
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1365<<<dim3(64,
// 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1360_0, Constant_179_0,
// DepthwiseConv2dNative_1365_0); Add_float_float_float_cuda_Add_1366<<<dim3(16,
// 1, 1), dim3(512, 1, 1), 0, 0>>>(AvgPool_1361_0, AvgPool_1361_0, Add_1366_0);
// Convolution_float_float_float_cuda_Convolution_1379<<<dim3(1, 4, 16), dim3(8,
// 2, 8), 0, 0>>>(DepthwiseConv2dNative_1374_0, Constant_2515_0,
// Convolution_1379_0); Deduped function map: <src_function_name :
// deduped_function_name>
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1365 :
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1364
...
// 128, 8, 8}
__device__ __forceinline__ void
DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1363_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 128) {
    return;
...
// 128, 8, 8}
__device__ __forceinline__ void
DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1364_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 128) {
    return;
...
__device__ __forceinline__ void
fuse2_DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1364_block_kernel(
...
//	- name: Add_1366_0	type: float	shape: Shape{1, 128, 8, 8}
__device__ __forceinline__ void
Add_float_float_float_cuda_Add_1366_block_kernel(float *input0, float *input1,
...
__device__ __forceinline__ void
Fuse_AvgPool_1361_Add_1366(const float *input, float *output, int thread_id,
...
#pragma unroll 4
    ...
    // output
    output[index] = add(avgval, avgval);
  }
}
...
// 8}
__device__ __forceinline__ void
Convolution_float_float_float_cuda_Convolution_1379_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 128) {
    return;
...
extern "C" __global__ void
BlockFusionKernel_float_float_float_float_float_float_float_float_float_float_float_float_cuda_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_Add_Convolution_125(
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 15) {
    Fuse_AvgPool_1361_Add_1366(input4, output3, threadIdx.x, blockIdx.x - 0,
                               shared_buffer);
    // Add_float_float_float_cuda_Add_1366_block_kernel(input4, input4, output3,
    // threadIdx.x, blockIdx.x - 192 + 0, shared_buffer);
  } else if ((int)blockIdx.x >= 80 + 64 && (int)blockIdx.x <= 143 + 64)
  // else if((int)blockIdx.x >= 16 && (int)blockIdx.x <= 79)
  {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1363_block_kernel(
        input0, input1, output0, threadIdx.x, blockIdx.x - 80 - 64,
        shared_buffer);
  } else if ((int)blockIdx.x >= 16 + 64 && (int)blockIdx.x <= 79 + 64) {
    fuse2_DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1364_block_kernel(
        input0, input2, input3, output1, output2, threadIdx.x,
        blockIdx.x - 16 - 64, shared_buffer);
    // DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1364_block_kernel(input0,
    // input2, output1, threadIdx.x, blockIdx.x - 80, shared_buffer);
  }
  // else if((int)blockIdx.x >= 144 && (int)blockIdx.x <= 207)
  // {
  //     DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1364_block_kernel(input0,
  //     input3, output2, threadIdx.x, blockIdx.x - 144, shared_buffer);
  // }
  else if ((int)blockIdx.x >= 0 + 16 && (int)blockIdx.x <= 64 + 16) {
    Convolution_float_float_float_cuda_Convolution_1379_block_kernel(
        input5, input6, output4, threadIdx.x, blockIdx.x - 16, shared_buffer);
  }
}
...
// Fused functions:
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1734<<<dim3(64,
// 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1732_0, Constant_100_0,
// DepthwiseConv2dNative_1734_0);
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1735<<<dim3(64,
// 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1732_0, Constant_360_0,
// DepthwiseConv2dNative_1735_0); Add_float_float_float_cuda_Add_1736<<<dim3(16,
// 1, 1), dim3(512, 1, 1), 0, 0>>>(AvgPool_1733_0, BatchNormInference_1670_0,
// Add_1736_0); Deduped function map: <src_function_name :
// deduped_function_name>
...
// 128, 8, 8}
__device__ __forceinline__ void
DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1734_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 128) {
    return;
...
// 128, 8, 8}
__device__ __forceinline__ void
DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1735_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 128) {
    return;
...
//	- name: Add_1736_0	type: float	shape: Shape{1, 128, 8, 8}
__device__ __forceinline__ void
Add_float_float_float_cuda_Add_1736_block_kernel(float *input0, float *input1,
                                                 float *output0, int thread_id,
                                                 int block_id,
                                                 char *shared_buffer) {
  if (thread_id >= 512) {
...
__device__ __forceinline__ void
Fuse_AvgPool_1361_Add_1736(const float *input0, const float *input1,
...
extern "C" __global__ void
BlockFusionKernel_float_float_float_float_float_float_float_float_cuda_DepthwiseConv2dNative_DepthwiseConv2dNative_Add_175(
    float *input0, float *input1, float *input2, float *input3, float *input4,
    float *output0, float *output1, float *output2) {

  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 15) {
    Fuse_AvgPool_1361_Add_1736(input3, input4, output2, threadIdx.x,
                               blockIdx.x - 0, NULL);
    // Add_float_float_float_cuda_Add_1736_block_kernel(input3, input4, output2,
    // threadIdx.x, blockIdx.x - 128 + 0, NULL);
  } else if ((int)blockIdx.x >= 16 && (int)blockIdx.x <= 79) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1734_block_kernel(
        input0, input1, output0, threadIdx.x, blockIdx.x - 16, NULL);
  } else if ((int)blockIdx.x >= 80 && (int)blockIdx.x <= 143) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1735_block_kernel(
        input0, input2, output1, threadIdx.x, blockIdx.x - 80, NULL);
  }
}
...
// Node name:	Constant_2909
// Description:	Constant
// Input:
// Output:
//	- name: Constant_2909_0	type: float	shape: Shape{1, 64, 16, 16}
void Constant_float_cuda_Constant_2909(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2909_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
    printf("Load Constant_2909_0 failed.\n");
    exit(1);
  }
  char *tmp_mem = new char[65536];
  bin_file.read(tmp_mem, 65536);
