...
//	- name: Slice_894_0	type: float	shape: Shape{1, 192, 32, 32}
__device__ __forceinline__ void
Fuse_Add_Slice_float_float_cuda_Slice_894_block_kernel(
  ...
  // const dim3 blockIdx(block_id, 0, 0);
  // if (thread_id < 171) {
  //     int id = blockIdx.x * 171 + threadIdx.x;
  //     if (id < 65536)
  //         output1[id] = add(input1[id], input2[id]);
  // }
  // output1[blockIdx.x * 512 + threadIdx.x] = add(input1[blockIdx.x * 512 +
  // threadIdx.x], input2[blockIdx.x * 512 + threadIdx.x]);
...
// B1lockFusionKernel_float_float_float_float_float_float_float_float_cuda_Add_Convolution_Slice_55(float*
// input0, float* input1, float* input2, float* input3, float* input4, float*
// output0, float* output1, float* output2)
// {
//     __shared__ char shared_buffer[2048];

//     if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 383)
//     {
//         Fuse_Add_Slice_float_float_cuda_Slice_894_block_kernel(input4,
//         input0, input1, output2, output0, threadIdx.x, blockIdx.x,
//         shared_buffer);
//     }
//     else if ((int)blockIdx.x >= 384 && (int)blockIdx.x <= 415)
//     {
//         Convolution_float_float_float_cuda_Convolution_893_block_kernel(input2,
//         input3, output1, threadIdx.x, blockIdx.x - 128 + 0, shared_buffer);
//     }
// }
...
BlockFusionKernel_float_float_float_float_float_float_float_float_cuda_Add_Convolution_Slice_55(
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 127) {
    ...
    Slice_float_float_cuda_Slice_894_block_kernel(
        input4, output2, threadIdx.x, blockIdx.x - 160, shared_buffer);
  }
}
