...
__device__ void operator_avg_pool_h_64_32_32_3x3_2(const float *input,
                                                   float *output,
                                                   int blockidx) {
...
// grid(192,1,1) block(256,1,1)
__device__ void operator_avg_pool_h_192_32_32_1x1_2(const float *input,
                                                    float *output,
                                                    int blockidx) {

  const int pooled_height = 16;
  const int pooled_width = 16;
  const int nthreads = 49152;
  int index = blockidx * 256 + threadIdx.x;

  if (index < nthreads) {
    const int kChannels = 192;
    const int kHeight = 32;
    const int kWidth = 32;
...
// 3, 0, 2(kernelH, pad, stride)
// grid(64,1,1) block(256,1,1)
__device__ void operator_max_pool_h_64_32_32_3x3_2(const float *input,
                                                   float *output,
                                                   int blockidx) {
...
extern "C" __global__ void BlockFusionKernel_2_AvgPool897_MaxPool898_AvgPool888(
    float *input0, float *input1, float *input2, float *output0, float *output1,
    float *output2) {

  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 63) {
    operator_avg_pool_h_64_32_32_3x3_2(input0, output0, blockIdx.x);
  } else if ((int)blockIdx.x >= 64 && (int)blockIdx.x <= 127) {
    operator_max_pool_h_64_32_32_3x3_2(input1, output1, blockIdx.x - 64);
  } else if ((int)blockIdx.x >= 128 && (int)blockIdx.x <= 319) {
    operator_avg_pool_h_192_32_32_1x1_2(input2, output2, blockIdx.x - 128);
  }
}
...
extern void BlockFusionKernel_2_AvgPool897_MaxPool898_AvgPool888_Call(
    ...
    float *input0, float *input1, float *input2, float *output0, float *output1,
    float *output2) {
  BlockFusionKernel_2_AvgPool897_MaxPool898_AvgPool888<<<grids, blocks, mem,
                                                         stream>>>(
