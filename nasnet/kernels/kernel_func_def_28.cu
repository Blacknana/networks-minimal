...
extern "C" __launch_bounds__(64) __global__
    void Sum_float_float_cuda_Sum_1757(float *input0, float *input1,
                                       float *output0) {
  ...
  for (int tidx = thread_idx; tidx < width; tidx += block_size) {
    ...
    val += fmaxf(0, input0[data_idx]);
  }
  val = reduceSum(val, thread_idx, block_size, shm);
  if (thread_idx == 0)
    output0[block_idx] = fdividef(val, input1[block_idx]);
}
extern void Sum_float_float_cuda_Sum_1757_Call(const dim3 &grids,
                                               ...
                                               float *output0) {
  Sum_float_float_cuda_Sum_1757<<<grids, blocks, mem, stream>>>(input0, input1,
                                                                output0);
}
