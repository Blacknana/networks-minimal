...
void Constant_float_cuda_Constant_426(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_426_0.bin",
...
void Constant_float_cuda_Constant_2852(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2852_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2578(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2578_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_135(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_135_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2861(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2861_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2296(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2296_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2311(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2311_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2845(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2845_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
void Constant_float_cuda_Constant_2786(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2786_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
...
// Fused functions:
// Add_float_float_float_cuda_Add_902<<<dim3(32, 1, 1), dim3(512, 1, 1), 0,
// 0>>>(AvgPool_897_0, BatchNormInference_873_0, Add_902_0);
// Add_float_float_float_cuda_Add_903<<<dim3(32, 1, 1), dim3(512, 1, 1), 0,
// 0>>>(MaxPool_898_0, BatchNormInference_874_0, Add_903_0);
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_904<<<dim3(128,
// 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_899_0, Constant_91_0,
// DepthwiseConv2dNative_904_0);
// Concat_float_float_float_cuda_Concat_905<<<dim3(32, 1, 1), dim3(512, 1, 1),
// 0, 0>>>(Convolution_893_0, Convolution_901_0, Concat_905_0); Deduped function
...
// Output:
//	- name: Add_902_0	type: float	shape: Shape{1, 64, 16, 16}
__device__ __forceinline__ void Add_float_float_float_cuda_Add_902_block_kernel(
...
DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_904_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 128) {
    return;
...
#pragma unroll
        for (int filter_col = 0; filter_col < filter_width; ++filter_col) {
          ...
          sum += fmaxf(0, static_cast<S>(__ldg(input + input_offset))) *
...
//	- name: Concat_905_0	type: float	shape: Shape{1, 64, 16, 16}
__device__ __forceinline__ void
Concat_float_float_float_cuda_Concat_905_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 512) {
    return;
...
BlockFusionKernel_float_float_float_float_float_float_float_float_float_float_float_float_cuda_Add_Add_DepthwiseConv2dNative_Concat_57(
    float *input0, float *input1, float *input2, float *input3, float *input4,
    float *input5, float *input6, float *input7, float *output0, float *output1,
    float *output2, float *output3) {

  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 31) {
    Add_float_float_float_cuda_Add_902_block_kernel(
        input0, input1, output0, threadIdx.x, blockIdx.x - 0, NULL);
  } else if ((int)blockIdx.x >= 32 && (int)blockIdx.x <= 63) {
    Add_float_float_float_cuda_Add_902_block_kernel(
        input2, input3, output1, threadIdx.x, blockIdx.x - 32, NULL);
  } else if ((int)blockIdx.x >= 64 && (int)blockIdx.x <= 95) {
    Concat_float_float_float_cuda_Concat_905_block_kernel(
        input6, input7, output3, threadIdx.x, blockIdx.x - 64, NULL);
    // Can be eliminated?
  } else if ((int)blockIdx.x >= 96 && (int)blockIdx.x <= 223) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_904_block_kernel(
        input4, input5, output2, threadIdx.x, blockIdx.x - 96, NULL);
  }
}
...
// Output:
//	- name: Convolution_813_0	type: float	shape: Shape{1, 32, 32,
//32}
//	- name: Convolution_815_0	type: float	shape: Shape{1, 32, 32,
//32}
// Fused functions:
// Convolution_float_float_float_cuda_Convolution_813<<<dim3(2, 16, 2), dim3(16,
// 2, 8), 0, 0>>>(DepthwiseConv2dNative_804_0, Constant_2218_0,
// Convolution_813_0);
// Convolution_float_float_float_cuda_Convolution_815<<<dim3(2, 16, 2), dim3(16,
// 2, 8), 0, 0>>>(DepthwiseConv2dNative_805_0, Constant_2221_0,
// Convolution_815_0); Deduped function map: <src_function_name :
// deduped_function_name> Convolution_float_float_float_cuda_Convolution_815 :
// Convolution_float_float_float_cuda_Convolution_813
...
//32}
__device__ __forceinline__ void
Convolution_float_float_float_cuda_Convolution_813_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 256) {
    return;
...
extern "C" __global__ void
BlockFusionKernel_float_float_float_float_float_float_cuda_Convolution_Convolution_43(
...
//	- name: Constant_280_0	type: float	shape: Shape{5, 5, 128, 1}
//	- name: Constant_376_0	type: float	shape: Shape{7, 7, 128, 1}
//	- name: Constant_377_0	type: float	shape: Shape{7, 7, 128, 1}
...
//64, 16, 16}
// Fused functions:
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1287<<<dim3(64,
// 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1283_0, Constant_280_0,
// DepthwiseConv2dNative_1287_0);
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1285<<<dim3(64,
// 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1283_0, Constant_376_0,
// DepthwiseConv2dNative_1285_0);
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1286<<<dim3(64,
// 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1283_0, Constant_377_0,
// DepthwiseConv2dNative_1286_0); Add_float_float_float_cuda_Add_2478<<<dim3(32,
// 1, 1), dim3(512, 1, 1), 0, 0>>>(Convolution_1280_0, Constant_2792_0,
// BatchNormInference_1282_0); Deduped function map: <src_function_name :
// deduped_function_name>
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1286 :
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1285
...
//128, 8, 8}
__device__ __forceinline__ void
DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1287_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 128) {
    return;
...
//128, 8, 8}
__device__ __forceinline__ void
DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1285_block_kernel(
    float *input0, float *input1, float *output0, int thread_id, int block_id,
    char *shared_buffer) {
  if (thread_id >= 128) {
    return;
...
__device__ __forceinline__ void
fuse2_DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1285_block_kernel(
...
//64, 16, 16}
__device__ __forceinline__ void
Add_float_float_float_cuda_Add_2478_block_kernel(float *input0, float *input1,
...
extern "C" __global__ void
BlockFusionKernel_float_float_float_float_float_float_float_float_float_float_cuda_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_Add_111(
    float *input0, float *input1, float *input2, float *input3, float *input4,
    float *input5, float *output0, float *output1, float *output2,
    float *output3) {

  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 31) {
    Add_float_float_float_cuda_Add_2478_block_kernel(
  ...
  {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1287_block_kernel(
        input0, input1, output0, threadIdx.x, blockIdx.x - 96, NULL);
  } else if ((int)blockIdx.x >= 32 && (int)blockIdx.x <= 95) {
    fuse2_DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1285_block_kernel(
        input0, input2, input3, output1, output2, threadIdx.x, blockIdx.x - 32,
        NULL);
    // DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1285_block_kernel(input0,
    // input2, output1, threadIdx.x, blockIdx.x - 96, NULL);
  }
  // else if((int)blockIdx.x >= 160 && (int)blockIdx.x <= 223)
  // {
  //     DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1285_block_kernel(input0,
  //     input3, output2, threadIdx.x, blockIdx.x - 160, NULL);
  // }
}
...
//128, 8, 8}
// Fused functions:
// FusedKernel_float_float_float_float_cuda_Add_Relu_65<<<dim3(16, 1, 1),
// dim3(512, 1, 1), 0, 0>>>(Convolution_1603_0, Constant_2758_0, Relu_1608_0,
// BatchNormInference_1606_0); Add_float_float_float_cuda_Add_2655<<<dim3(16, 1,
// 1), dim3(512, 1, 1), 0, 0>>>(Convolution_1605_0, Constant_2953_0,
// BatchNormInference_1607_0); Deduped function map: <src_function_name :
// deduped_function_name>
...
//128, 8, 8}
// Fused functions:
// Add_float_float_float_cuda_Add_2652<<<dim3(16, 1, 1), dim3(512, 1, 1), 0,
// 0>>>(Convolution_1603_0, Constant_2758_0, BatchNormInference_1606_0);
// Relu_float_float_cuda_Relu_1608<<<dim3(16, 1, 1), dim3(512, 1, 1), 0,
// 0>>>(BatchNormInference_1606_0, Relu_1608_0);
__device__ __forceinline__ void
FusedKernel_float_float_float_float_cuda_Add_Relu_65_block_kernel(
...
//128, 8, 8}
__device__ __forceinline__ void
Add_float_float_float_cuda_Add_2655_block_kernel(float *input0, float *input1,
                                                 float *output0, int thread_id,
                                                 int block_id,
                                                 char *shared_buffer) {
  if (thread_id >= 512) {
...
extern "C" __global__ void
BlockFusionKernel_float_float_float_float_float_float_float_cuda_fused_kernel_Add_158(
    float *input0, float *input1, float *input2, float *input3, float *output0,
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 15) {
    ...
    Add_float_float_float_cuda_Add_2655_block_kernel(
...
// Fused functions:
// Add_float_float_float_cuda_Add_1485<<<dim3(16, 1, 1), dim3(512, 1, 1), 0,
// 0>>>(AvgPool_1482_0, AvgPool_1482_0, Add_1485_0);
// Matched_Pattern_float_float_float_float_cuda_Matched_Pattern_3143<<<dim3(1,
// 4, 16), dim3(8, 2, 8), 0, 0>>>(DepthwiseConv2dNative_1488_0, Constant_2590_0,
// Constant_3144_0, Relu_1509_0);
// Matched_Pattern_float_float_float_float_cuda_Matched_Pattern_3139<<<dim3(1,
// 4, 16), dim3(8, 2, 8), 0, 0>>>(DepthwiseConv2dNative_1486_0, Constant_2584_0,
// Constant_3140_0, Relu_1507_0);
// Matched_Pattern_float_float_float_float_cuda_Matched_Pattern_3141<<<dim3(1,
// 4, 16), dim3(8, 2, 8), 0, 0>>>(DepthwiseConv2dNative_1487_0, Constant_2587_0,
// Constant_3142_0, Relu_1508_0); Relu_float_float_cuda_Relu_1490<<<dim3(16, 1,
// 1), dim3(512, 1, 1), 0, 0>>>(Slice_1484_0, Relu_1490_0); Deduped function
// map: <src_function_name : deduped_function_name>
// Matched_Pattern_float_float_float_float_cuda_Matched_Pattern_3139 :
// Matched_Pattern_float_float_float_float_cuda_Matched_Pattern_3143
// Matched_Pattern_float_float_float_float_cuda_Matched_Pattern_3141 :
// Matched_Pattern_float_float_float_float_cuda_Matched_Pattern_3143
...
//	- name: Add_1485_0	type: float	shape: Shape{1, 128, 8, 8}
__device__ __forceinline__ void
Add_float_float_float_cuda_Add_1485_block_kernel(float *input0, float *input1,
                                                 float *output0, int thread_id,
                                                 int block_id,
                                                 char *shared_buffer) {
  if (thread_id >= 512) {
...
// Node name:	Matched_Pattern_3143
...
//	- name: Relu_1509_0	type: float	shape: Shape{1, 128, 8, 8}
__device__ __forceinline__ void
Matched_Pattern_float_float_float_float_cuda_Matched_Pattern_3143_block_kernel(
...
//	- name: Relu_1490_0	type: float	shape: Shape{1, 128, 8, 8}
__device__ __forceinline__ void
Relu_float_float_cuda_Relu_1490_block_kernel(float *input0, float *output0,
...
BlockFusionKernel_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_cuda_Add_Matched_Pattern_Matched_Pattern_Matched_Pattern_Relu_142(
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 15) {
    Add_float_float_float_cuda_Add_1485_block_kernel(
        input0, input0, output0, threadIdx.x, blockIdx.x - 0, shared_buffer);
  } else if ((int)blockIdx.x >= 16 && (int)blockIdx.x <= 31) {
    Relu_float_float_cuda_Relu_1490_block_kernel(
...
void Constant_float_cuda_Constant_2851(cudaStream_t stream, float *output0) {
  std::ifstream bin_file("./Constant/Constant_2851_0.bin",
                         std::ios::in | std::ios::binary);
  if (bin_file.fail()) {
