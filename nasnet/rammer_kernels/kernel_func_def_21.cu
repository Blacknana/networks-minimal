...
BlockFusionKernel_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_float_cuda_Convolution_DepthwiseConv2dNative_DepthwiseConv2dNative_Convolution_Convolution_DepthwiseConv2dNative_Convolution_0(
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 63) {
    Convolution_float_float_float_cuda_Convolution_471_block_kernel(
        ...
        shared_buffer);
  } else if ((int)blockIdx.x >= 64 && (int)blockIdx.x <= 831) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_480_block_kernel(
        input2, input3, output1, threadIdx.x, blockIdx.x - 64 + 0,
        shared_buffer);
  } else if ((int)blockIdx.x >= 832 && (int)blockIdx.x <= 1599) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_479_block_kernel(
        input2, input4, output2, threadIdx.x, blockIdx.x - 832 + 0,
        shared_buffer);
  } else if ((int)blockIdx.x >= 1600 && (int)blockIdx.x <= 1663) {
    Convolution_float_float_float_cuda_Convolution_471_block_kernel(
        input2, input5, output3, threadIdx.x, blockIdx.x - 1600 + 0,
        shared_buffer);
  } else if ((int)blockIdx.x >= 1664 && (int)blockIdx.x <= 1727) {
    Convolution_float_float_float_cuda_Convolution_471_block_kernel(
        input2, input6, output4, threadIdx.x, blockIdx.x - 1664 + 0,
        shared_buffer);
  } else if ((int)blockIdx.x >= 1728 && (int)blockIdx.x <= 2495) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_479_block_kernel(
        input2, input7, output5, threadIdx.x, blockIdx.x - 1728 + 0,
        shared_buffer);
  } else if ((int)blockIdx.x >= 2496 && (int)blockIdx.x <= 2559) {
    Convolution_float_float_float_cuda_Convolution_471_block_kernel(
        input2, input8, output6, threadIdx.x, blockIdx.x - 2496 + 0,
        shared_buffer);
  }
}
extern void
...
BlockFusionKernel_float_float_float_float_float_float_float_cuda_Add_fused_kernel_167(
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 15) {
    ...
    FusedKernel_float_float_float_float_cuda_Add_Relu_69_block_kernel(
        input3, input2, output2, output1, threadIdx.x, blockIdx.x - 16 + 0,
        NULL);
  }
}
extern void
