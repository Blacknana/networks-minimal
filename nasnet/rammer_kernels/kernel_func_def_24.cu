...
//	- name: Add_916_0	type: float	shape: Shape{1, 64, 16, 16}
__device__ __forceinline__ static void
Add_float_float_float_cuda_Add_916_block_kernel(float *input0, float *input1,
...
BlockFusionKernel_float_float_float_float_float_float_float_float_float_float_float_float_cuda_Convolution_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_Add_60(
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 63) {
    ...
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_914_block_kernel(
        input2, input4, output2, threadIdx.x, blockIdx.x - 192 + 0,
        shared_buffer);
  } else if ((int)blockIdx.x >= 320 && (int)blockIdx.x <= 447) {
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_914_block_kernel(
        input2, input5, output3, threadIdx.x, blockIdx.x - 320 + 0,
        shared_buffer);
  } else if ((int)blockIdx.x >= 448 && (int)blockIdx.x <= 479) {
    Add_float_float_float_cuda_Add_916_block_kernel(
        input6, input6, output4, threadIdx.x, blockIdx.x - 448 + 0,
        shared_buffer);
  }
}
extern void
