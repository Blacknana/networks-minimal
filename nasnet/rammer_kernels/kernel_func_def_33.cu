...
BlockFusionKernel_float_float_float_float_float_float_float_float_float_float_float_float_cuda_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_Add_Convolution_125(
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 63) {
    ...
    DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1364_block_kernel(
        input0, input3, output2, threadIdx.x, blockIdx.x - 128 + 0,
        shared_buffer);
  } else if ((int)blockIdx.x >= 192 && (int)blockIdx.x <= 207) {
    Add_float_float_float_cuda_Add_1366_block_kernel(
        input4, input4, output3, threadIdx.x, blockIdx.x - 192 + 0,
        shared_buffer);
  } else if ((int)blockIdx.x >= 208 && (int)blockIdx.x <= 271) {
    Convolution_float_float_float_cuda_Convolution_1379_block_kernel(
        input5, input6, output4, threadIdx.x, blockIdx.x - 208 + 0,
        shared_buffer);
  }
}
extern void
...
BlockFusionKernel_float_float_float_float_float_float_float_float_cuda_DepthwiseConv2dNative_DepthwiseConv2dNative_Add_175(
  ...
  if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 63) {
    ...
    Add_float_float_float_cuda_Add_1736_block_kernel(
        input3, input4, output2, threadIdx.x, blockIdx.x - 128 + 0, NULL);
  }
}
extern void
