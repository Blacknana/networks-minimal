...
static constexpr uint64_t AvgPoolArgSize() {
...
template <uint64_t kBatchSize, uint64_t kChannels, uint64_t kHeight,
          ...
          uint64_t kPadW, uint64_t kStrideH, uint64_t kStrideW>
class AvgPool
    : public Op<AvgPoolArgSize<kBatchSize, kChannels, kHeight, kWidth, kKernelH,
                               kKernelW, kPadH, kPadW, kStrideH, kStrideW>()> {
public:
  explicit AvgPool(void *device_ptr) {
  ...
  template <int T> void Forward(Tensor<T> *input) {
    ...
    cudaLaunchKernel(
        (const void *)operator_avg_pool_h<kBatchSize, kChannels, kHeight,
...
private:
  static constexpr uint64_t kSize =
      AvgPoolArgSize<kBatchSize, kChannels, kHeight, kWidth, kKernelH, kKernelW,
                     kPadH, kPadW, kStrideH, kStrideW>();
  using Parent = Op<kSize>;
  AvgPoolState<kSize> *state_;
};

// template class AvgPool<1, 2048, 7, 7, 7, 7, 0, 0, 7, 7>;